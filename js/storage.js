Config.origindomain = 'main.pokemon-world.online';
...
Storage.bg = {
	...
	load: function (bgUrl, bgid, hues) {
		...
		if (!bgid) {
			if (location.host === '') {
				bgid = 'shaymin';
			} else if (location.host === 'main.pokemon-world.online') {
				bgid = ['', '', '', '', ''][Math.floor(Math.random() * 5)];
			} else {
				$(document.body).css({
					background: '',
					'background-size': ''
		...
		if (bgUrl.charAt(0) === '#') {
			...
			background = '#ffffff url(' + bgUrl + ') no-repeat left center fixed';
		} else {
			background = '#ffffff url(' + bgUrl + ') no-repeat center center fixed';
		}
	/*	$(document.body).css({
...
// in the origin https://main.pokemon-world.online
...
Storage.origin = 'https://main.pokemon-world.online';
...
Storage.initPrefs = function () {
	...
	if (document.location.hostname !== Config.origindomain) {
		$(
			'<iframe src="https://main.pokemon-world.online/crossdomain.php?host=' +
		...
		Config.server = Config.server || Config.defaultserver;
		$(
			'<iframe src="https://main.pokemon-world.online/crossprotocol.html?v1.2" style="display: none;"></iframe>'
		).appendTo('body');
		setTimeout(function () {
	...
	case 'c':
		...
		if (Config.server.registered && Config.server.id !== 'showdown' && Config.server.id !== 'smogtours') {
			var $link = $('<link rel="stylesheet" ' +
				'href="//main.pokemon-world.online/customcss.php?server=' +
 ...
 * If the client isn't running on `main.pokemon-world.online`, though,
