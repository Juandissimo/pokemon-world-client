...
if (window.soundManager) {
	soundManager.setup({url:'https://main.pokemon-world.online/swf/'});
	if (window.Replays) soundManager.onready(window.Replays.soundReady);
	soundManager.onready(function () {
		soundManager.createSound({
			id: 'notif',
			url: 'https://main.pokemon-world.online/audio/notification.wav'
		});
	});
}
...
var Tools = {
	...
	resourcePrefix: (function () {
		...
		return prefix + '//main.pokemon-world.online/';
	})(),

	fxPrefix: (function () {
		if (document.location.protocol === 'file:') {
			if (window.Replays) return 'https://main.pokemon-world.online/fx/';
			return 'fx/';
		}
		return '//main.pokemon-world.online/fx/';
	})(),
	...
	parseChatMessage: function (message, name, timestamp, isHighlighted) {
		...
		case 'data-pokemon':
			...
			buf += '<span class="col pokemonnamecol" style="white-space:nowrap"><a href="https://pokemon-world.online/dex/pokemon/' + template.id + '" target="_blank">' + template.species + '</a></span> ';
	...
	interstice: (function () {
		...
		return {
			...
			getURI: function (uri) {
				return 'http://pokemon-world.online/interstice?uri=' + encodeURIComponent(uri);
			}
		};
	})(),
	...
	createReplayFile: function (room) {
		...
		buf += '<h1 style="font-weight:normal;text-align:center"><strong>' + Tools.escapeHTML(battle.tier) + '</strong><br /><a href="http://pokemon-world.online/users/' + toId(battle.p1.name) + '" class="subtle" target="_blank">' + Tools.escapeHTML(battle.p1.name) + '</a> vs. <a href="http://pokemon-world.online/users/' + toId(battle.p2.name) + '" class="subtle" target="_blank">' + Tools.escapeHTML(battle.p2.name) + '</a></h1>\n';
		...
		buf += '</div>\n';
		buf += '<script>\n';
		buf += 'var daily = Math.floor(Date.now()/1000/60/60/24);document.write(\'<script src="https://main.pokemon-world.online/js/replay-embed.js?version\'+daily+\'"></\'+\'script>\');\n';
		buf += '</script>\n';
		return buf;
	},
