(function ($) {
	...
	var MainMenuRoom = this.MainMenuRoom = this.Room.extend({
		...
		initialize: function () {
			...
			if (app.down) {
				...
				if (app.down === 'ddos') {
					buf += '<p class="error"><strong>Pok&eacute;mon World is offline due to a DDoS attack!</strong></p>';
				} else {
					buf += '<p class="error"><strong>Pok&eacute;mon World is offline due to technical difficulties!</strong></p>';
				}
				buf += '<p><div style="text-align:center"><img width="96" height="96" src="//main.pokemon-world.online/sprites/bw/teddiursa.png" alt="" /></div> be back soon.</p>';
				buf += '<p>(We\'ll be back up in a few hours.)</p>';
				buf += '</div>';
			} else {
				buf += '<div class="menugroup"><form class="battleform" data-search="1">';
				buf += '<p><label class="label">Generation:</label>' + this.renderFormats() + '</p>';
				buf += '<p><label class="label">My Team:</label>' + this.renderTeams() + '</p>';
				buf += '<p><button class="button mainmenu1 big" name="search"><strong>Find a battle!</strong><br /><small>Find a battle!</small></button></p></form></div>';
			}

			buf += '<div class="menugroup"><p><button class="button mainmenu2" name="joinRoom" value="teambuilder"><strong style="color:white">Build a Team</button></p>';
			buf += '<p><button class="button mainmenu3" name="joinRoom" value="ladder"><strong style="color:white">Leaderboard</button></p></div>';

			buf += '<div class="menugroup"><p><button class="button mainmenu4 onlineonly disabled" name="joinRoom" value="battles"><strong style="color:white">Live Battles</button></p>';
			buf += '<p><button class="button mainmenu5 onlineonly disabled" name="finduser"><strong style="color:white">Find a player</button></p></div>';

			this.$('.mainmenu').html(buf);

			// right menu
			if (document.location.hostname === 'main.pokemon-world.online') {
				this.$('.rightmenu').html('<div class="menugroup1"><p><button class="button mainmenu1 onlineonly disabled" name="joinRoom" value="rooms"><font color="white">Main Menu</button></p></div>');
			} else {
				this.$('.rightmenu').html('<div class="menugroup"><p><button class="button mainmenu5 onlineonly disabled" name="joinRoom" value="lobby"><strong style="color:green">Login In</button></p></div>');
			}
		...
		addPM: function (name, message, target) {
			...
			if (!isSelf && textContent) {
				this.notifyOnce("Message from " + name, "\"" + textContent + "\"", 'pm');
			}
		...
		submitPM: function (e) {
			...
			if (text.toLowerCase() === '/ignore') {
				if (app.ignore[userid]) {
					...
					app.ignore[userid] = 1;
					$chat.append('<div class="chat">User ' + userid + ' ignored. (Staff messages will not be ignored.)</div>');
				}
				...
				text = ('\n' + text).replace(/\n\n/g, '\n').replace(/\n/g, '\n/pm ' + userid + ', ').substr(1);
				if (text.length > 500) {
					app.addPopupMessage("PM\'s are disabled. Until we finish development ");
					return;
				}
				this.send(text);
			}
		...
		updateSearch: function (data) {
			...
			if (!this.searching || $.isArray(this.searching) && !this.searching.length) {
				...
				$searchForm.find('button.big').html('<strong>Find a battle!</strong><br /><small>Find a random opponent</small>').removeClass('disabled');
				...
				$searchForm.find('button.big').html('<strong><i class="fa fa-refresh fa-spin"></i> Finding...</strong>').addClass('disabled');
		...
		updateChallenges: function (data) {
			...
			if (data.challengeTo) {
				...
				var buf = '<form class="battleform"><p>Connecting ' + Tools.escapeHTML(name) + '...</p>';
				buf += '<p><label class="label">Format:</label>' + this.renderFormats(challenge.format, true) + '</p>';
				buf += '<p class="buttonbar"><button name="cancelChallenge">Cancel (confirm)</button></p></form>';
			...
			this.$('.pm-window').each(function (i, el) {
				...
				if (data.challengesFrom[userid]) {
					...
					var buf = '<form class="battleform"><p>' + Tools.escapeHTML(name) + ' wants to challenge you!</p>';
					...
					buf += '<p class="buttonbar"><button name="acceptChallenge"><strong>Accept (confirm)</strong></button> <button name="rejectChallenge">Reject (confirm)</button></p></form>';
					$challenge.html(buf);
		...
		updateFormats: function () {
			if (!window.BattleFormats) {
				...
				$searchForm.append('<p class="cancel buttonbar"><button name="reconnect">Rejoin battle!</button></p>');
				this.$('button.onlineonly').addClass('disabled');
				return;
			}
			this.$('button.onlineonly').removeClass('disabled');

			if (!this.searching) this.$('.mainmenu button.big').html('<strong>Find a battle!</strong><br /><small>challenge random users</small>').removeClass('disabled');
			var self = this;
		...
		renderFormats: function (formatid, noChoice) {
			if (!window.BattleFormats) {
				return '<button class="select formatselect" name="format" disabled value="' + Tools.escapeHTML(formatid) + '"><em>Connecting...</em></button>';
			}
		...
		renderTeams: function (formatid, teamIndex) {
			if (!Storage.teams || !window.BattleFormats) {
				return '<button class="select teamselect" name="team" disabled><em>Connecting...</em></button>';
			}
			...
			var teams = Storage.teams;
			if (!teams.length) {
				return '<button class="select teamselect" name="team" disabled>Please create a team</button>';
			}
		...
		search: function (i, button) {
			...
			if (!window.BattleFormats[format].team && (teamIndex === '' || !team)) {
				if (Storage.teams) {
					app.addPopupMessage("Please choose a team.");
				} else {
					app.addPopupMessage("Go to Build A Team. and create your ultimate POKEMON partners.");
				}
				return;
			}
		...
		finduser: function () {
			...
			app.addPopupPrompt("Desired name", "Open", function (target) {
				if (!target) return;
				if (toId(target) === 'adam') {
					app.addPopup(Popup, {htmlMessage: "im busy atm"});
					return;
				}
	...
	var TeamPopup = this.TeamPopup = this.Popup.extend({
		...
		renderTeam: function (i) {
			if (i === 'random') {
				var buf = '<strong> You have no team partners.</strong><small>';
				for (var i = 0; i < 6; i++) {
