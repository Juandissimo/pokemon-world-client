...
class DefaultActionHandler {
	...
	public function register($dispatcher, &$reqData, &$out) {
		...
		if ((mb_strlen($userid) < 1) || ctype_digit($userid)) {
			$out['actionerror'] = 'Your name must contain at least one letter.';
		} else if (substr($userid, 0, 5) === 'host') {
			$out['actionerror'] = 'You can\'t register host \'blocked\'.';
		} else if (mb_strlen($user['username']) > 19) {
			$out['actionerror'] = 'Your username must be less than 19 characters long.';
		} else if (mb_strlen(@$_POST['password']) < 8) {
			$out['actionerror'] = 'Your password has to be 8 characters.';
		} else if (@$_POST['password'] !== @$_POST['cpassword']) {
			$out['actionerror'] = 'Your pwssword\'s do not match.';
		} else if (trim(strtolower(@$_POST['captcha'])) !== 'milotic') {
			$out['actionerror'] = 'Who\'s that POKEMON.';
			...
			$out['actionerror'] = 'Access Denied: You can register only one name every 24 hours';
			...
			$out['actionerror'] = 'This name has been taken.';
		}
	}
	...
	public function changepassword($dispatcher, &$reqData, &$out) {
		...
		if (!$_POST ||
			...
			$out['actionerror'] = 'Your new password must be at least 8 characters long.';
	...
	public function logout($dispatcher, &$reqData, &$out) {
		...
		setcookie('showdown_username', '', time()-60*60*24*2, '/', 'localhost:8081');
		$out['actionsuccess'] = true;
	}
	...
	public function prepreplay($dispatcher, &$reqData, &$out) {
		...
		include_once __DIR__.'/../../replay.pokemon-world.online/replays.lib.php';
	...
	public function uploadreplay($dispatcher, &$reqData, &$out) {
		...
		include __DIR__.'/../../replay.pokemon-world.online/replays.lib.php';
	...
	public function invalidatecss($dispatcher, &$reqData, &$out) {
		...
		$cssfile = dirname(__FILE__) . '/../../pokemon-world.online/config/customcss/' . $server['id'] . '.css';
		@unlink($cssfile);
	}
