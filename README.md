...
This is the source code for the game server of [Pokémon World][4], a website for Pokémon battling. Pokémon World simulates singles, doubles and triples battles in all the games out so far (Generations 1 through 6).

This repository contains the files needed to set up your own Pokémon World server. Note that to set up a server, you'll also need a server computer.
...
You will be redirected to `http://SERVER.psim.online`. The reason your server is visited through `psim.online` is to make it more difficult for servers to see a user's password in any form, by handling logins globally. You can embed this in an `iframe` in your website if the URL is a big deal with you.
...
- See http://pokemon-world.online/credits
