(function ($) {
	...
	var User = this.User = Backbone.Model.extend({
		 ...
		 * to this file will always be made on the `testclient.pokemon-world.online`
		...
		finishRename: function (name, assertion) {
			...
			if (assertion.indexOf('<') >= 0) {
				...
				app.addPopupMessage("Mayhem has disabled the login server.");
		...
		logout: function () {
			...
			app.send('/logout');
			app.trigger('init:socketclosed', "You have signed out.<br /><br />If you wanted to change your name while staying connected, use the 'Desired name' button or the '/nick' command.", false);
			app.socket.close();
		},
		setPersistentName: function (name) {
			if (location.host !== 'testclient.pokemon-world.online') return;
	...
	var App = this.App = Backbone.Router.extend({
		...
		initialize: function () {
			...
			// down
			// if (document.location.hostname === 'main.pokemon-world.online') this.down = 'dos';
			...
			if (this.down) {
				...
				if (document.location.hostname === 'testclient.pokemon-world.online' || Config.testclient) {
				...
				Storage.whenPrefsLoaded(function () {
					...
					if (typeof autojoin === 'string') {
						// Use the existing autojoin string for pwo, and an empty string for other servers.
						if (Config.server.id !== 'simulator') autojoin = '';
					} else {
					...
					if (autojoin) {
						...
						for (var i = 0; i < autojoins.length; i++) {
							...
							if (Config.server.id !== 'simulator' && roomid === 'lobby') continue;
			...
			this.on('init:socketclosed', function (message, showNotification) {
				...
				if (showNotification !== false && (self.popups.length || !self.focused) && window.Notification) {
					...
					var disconnect = new Notification("Disconnected!", {lang: 'en', body: "You have been disconnected from Pokémon World"});
					disconnect.onclick = function (e) {
						window.focus();
					};
				}
				self.rooms[''].updateFormats();
				$('.pm-log-add form').html('<small>TestClient chat offline.</small>');
				$('.chat-log-add').html('<small>TestClient chat offline.</small>');
			...
			$(window).on('beforeunload', function (e) {
				...
				for (var id in self.rooms) {
					...
					if (room && room.requestLeave && !room.requestLeave()) return "You have on going battles.";
				}
			});
		...
		connect: function () {
			...
			if (Config.server.banned || bannedHosts.indexOf(Config.server.host) >= 0) {
				this.addPopupMessage("Your server has been 'banned' for breaching are hosting policy. Please contact the main owners (~) at http://main.pokemon-world.online to discuss your ban appeal. demanding a server side unban will result in being permanently banned");
				return;
			}
				...
				if (data === 'namerequired') {
					...
					if (errormessage) {
						if (data === 'nonexistent' && Config.server.id && roomid.slice(0, 7) === 'battle-') {
							...
							if (Config.server.id !== 'simulator') replayid = Config.server.id + '-' + replayid;
							var replayLink = 'http://replay.pokemon-world.online/' + replayid;
			...
			for (var j = 1; j < formatsList.length; j++) {
				if (isSection) {
					...
					if (isTeambuilderFormat) {
						...
						if (id.slice(0, 3) !== 'gen') {
							teambuilderFormatName = '[Generation 6] ' + name;
						}
		...
		uploadReplay: function (data) {
			...
			if (serverid && serverid !== 'simulator') id = serverid + '-' + id;
			$.post(app.user.getActionPHP() + '?act=uploadreplay', {
				log: data.log,
				id: id
			}, function (data) {
				if ((serverid === 'simulator') && (data === 'invalid id')) {
					data = 'not found';
				}
				if (data === 'success') {
					...
					app.addPopupMessage("Your server isn't registered, and doesn't support uploading replays.");
		...
		clearGlobalListeners: function () {
			...
			$(document).on('click', 'a', function (e) {
				...
				if (this.host === 'main.pokemon-world.online' || this.host === 'psim.online' || this.host === location.host) {
		...
		resize: function () {
			...
			if (!app.roomsFirstOpen && !this.down && $(window).width() >= 916 && document.location.hostname === 'main.pokemon-world.online') {
				this.addRoom('rooms');
			}
			this.updateLayout();
		},
		...
		updateTitle: function (room) {
			document.title = room.title ? room.title + " - PWO!" : "PWO!";
		},
		updateAutojoin: function () {
			...
			for (var i = 0; i < rooms.length; i++) {
				...
				if (room.id === 'staff' || room.id === 'upperstaff' || (Config.server.id !== 'simulator' && room.id === 'lobby')) continue;
				autojoinCount++;
			...
			if (typeof curAutojoin !== 'string') {
				...
				if (!autojoins.length) {
					...
					// If the only key left is 'simulator', revert to the string method for storing autojoin.
				...
				if (Config.server.id !== 'simulator') {
					// Switch to the autojoin object to handle multiple servers
					curAutojoin = {simulator: curAutojoin};
	...
	var UserPopup = this.UserPopup = Popup.extend({
		...
		update: function (data) {
			...
			var groupDetails = {
				...
				'\u2605': "Player (\u2605)",
				'+': "Voice (+)",
				'‽': "<span style='color:#777777'>Restricted</span>",
				'✖': "<span style='color:#777777'>Name Restriction</span>",
			...
			buf += '<strong><a href="//pokemon-world.online/users/' + userid + '" target="_blank">' + Tools.escapeHTML(name) + '</a></strong><br />';
				...
				for (var i in data.rooms) {
					...
					if (roomid.substr(0, 7) === 'battle-') {
						...
						if (data.rooms[i].isPrivate) {
							if (!privatebuf) privatebuf = '<br /><em>Private Channels:</em> ';
							else privatebuf += ', ';
							privatebuf += room;
						} else {
							if (!battlebuf) battlebuf = '<br /><em>Live Battles:</em> ';
							else battlebuf += ', ';
							battlebuf += room;
						}
					} else {
						...
						if (data.rooms[i].isPrivate) {
							...
							if (!chatbuf) chatbuf = '<br /><em>Channels:</em> ';
							else chatbuf += ', ';
							chatbuf += room;
						}
					}
			...
			buf += '<p class="buttonbar"><img src="https://img.pokemondb.net/sprites/black-white/anim/normal/dragonite.gif" width="50" height="50"> ';
			if (userid === app.user.get('userid') || !app.user.get('named')) {
				buf += '<button disabled>Battle!</button> - <button disabled>PM</button>';
				if (userid === app.user.get('userid')) {
					buf += '</p><hr /><p class="buttonbar" style="text-align: right">';
					buf += '<button name="login"><i class="fa fa-pencil"></i> Desired name</button> <button name="logout"><i class="fa fa-power-off"></i> Sign out</button>';
				}
			} else {
				buf += '<button name="challenge">Battle!</button> - <button name="pm">PM</button> <button name="userOptions">\u2026</button>';
			}
			buf += '</p>';
	...
	var UserOptionsPopup = this.UserOptions = Popup.extend({
		...
		toggleIgnoreUser: function () {
			...
			if (app.ignore[this.userid]) {
				...
				buf += " ignored. (Staff messages will not be ignored.)";
			}
	...
	var ProxyPopup = this.ProxyPopup = Popup.extend({
		...
		initialize: function (data) {
			...
			var buf = '<form>';
			buf += '<p>Due to the same <a href="https://en.wikipedia.org/wiki/Same-origin_policy" target="_blank">code policy</a> you are required to complete the requested action when using <code>testclient.html</code>.</p>';
			buf += '<iframe id="overlay_iframe" src="' + data.uri + '" style="width: 100%; height: 50px;" class="textbox"></iframe>';
			buf += '<p>You have to copy <strong>all the text</strong> from the box above and paste it in the box below.</p>';
			buf += '<p><label class="label" style="float: left;">Action data from the box above:</label> <input style="width: 100%;" class="textbox autofocus" type="text" name="result" /></p>';
			buf += '<p class="buttonbar"><button type="submit"><strong>Enter</strong></button> <button name="close">Cancel</button></p>';
			buf += '</form>';
	...
	var ReplayUploadedPopup = this.ReplayUploadedPopup = Popup.extend({
		...
		initialize: function (data) {
			...
			buf += '<p><a href="http://replay.pokemon-world.online/' + data.id + '" target="_blank">http://replay.pokemonshowdown.com/' + data.id + '</a></p>';
		...
		submit: function (i) {
			app.openInNewWindow('http://replay.pokemon-world.online/' + this.id);
			this.close();
		}
	});
	...
	var RulesPopup = this.RulesPopup = Popup.extend({
		...
		initialize: function (data) {
			...
			buf += '<h2>Pok&eacute;mon World Rules</h2>';
