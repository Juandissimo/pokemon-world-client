...
class NTBBSession {
	var $cookiedomain = 'pokemon-world.online';
	...
	private function passwordVerifyInner($userid, $pass, $user) {
		...
		// throttle login for php by mayhem
		$ip = $this->getIp();
		$res = $psdb->query(
			"SELECT `count`, `time` " .
	...
	function getAssertion($userid, $serverhostname, $user = null, $challengekeyid = -1, $challenge = '', $challengeprefix = '') {
		...
		if (substr($userid, 0, 7) === 'xotic') {
			return ';;A database error occured.';
		} else if (strlen($userid) > 19) {
			return ';; ignored';
		...
		if (($user['userid'] === $userid) && !empty($user['loggedin'])) {
			...
			if (in_array($userid, $psconfig['sysops'], true)) {
				...
				if ($forceUsertype) {
					...
					if ($serverhostname === 'sim.psim.online') {
						$usertype = '40';
					} else {
						$usertype = '2';
					}
	...
	function getRecentRegistrationCount($ip = '', $timeperiod = 7800 /* 2 hours */) {
		global $psdb;
		if ($ip === '') {
			$ip = $this->getIp();
		}
