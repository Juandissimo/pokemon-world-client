	...
	var ConsoleRoom = this.ConsoleRoom = Room.extend({
		...
		updateUser: function () {
			...
			if (this.expired) {
				...
				this.$chatAdd.html('<form><button name="login">Sign In</button></form>');
				this.$chatbox = null;
			} else {
		...
		handleTabComplete: function ($textbox) {
			// Dont tab complete at the start of the text box.
			var idx = $textbox.prop('selectionStart');
			...
			if (this.tabComplete.cursor !== null && text.substr(0, idx) === this.tabComplete.cursor) {
				...
				// dont include command names in autocomplete
			...
			case 'ladder':
				...
				$.get(app.user.getActionPHP(), {
					...
					if (registered && registered.userid === userid) {
						buffer += '<tr><td colspan="8" style="text-align:right"><a href="//pokemon-world.online/users/' + userid + '">Reset W/L</a></tr></td>';
					}
					buffer += '</table></div>';
					self.add('|raw|' + buffer);
				}), 'text');
