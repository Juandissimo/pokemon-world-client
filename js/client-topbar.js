(function ($) {
	...
	var Topbar = this.Topbar = Backbone.View.extend({
		...
		initialize: function () {
			// April Fool's 2016 - Digimon World
			// this.$el.html('<img class="logo" src="' + Tools.resourcePrefix + '" alt="" width="146" height="44" /><div class="maintabbarbottom"></div><div class="tabbar maintabbar"><div class="inner"></div></div><div class="userbar"></div>');
			this.$el.html('<img class="logo" src="' + Tools.resourcePrefix + '" srcset="' + Tools.resourcePrefix + '" alt="" width="146" height="44" /><div class="maintabbarbottom"></div><div class="tabbar maintabbar"><div class="inner"></div></div><div class="userbar"></div>');
		...
		updateUserbar: function () {
			...
			if (!app.user.loaded) {
				buf = '<button disabled>Connecting... <i class="icon-spin"> </i></button>';
				...
				buf = '<button name="login">Login Details</button>';
			}
		...
		renderRoomTab: function (room, id) {
			...
			switch (id) {
			case '':
				return buf + '> <img src="https://img.pokemondb.net/sprites/black-white/anim/normal/gastly.gif" width="40" height="40"></i> <span></span></a></li>';
			case 'teambuilder':
			...
			case 'ladder':
				return buf + '><i class="fa fa-list-ol"></i> <span>Leaderboard</span></a><button class="closebutton" name="closeRoom" value="' + 'ladder"><i class="fa fa-times-circle"></i></button></li>';
			case 'battles':
				return buf + '><i class="fa fa-caret-square-o-right"></i> <span>Active Battles</span></a><button class="closebutton" name="closeRoom" value="' + 'battles"><i class="fa fa-times-circle"></i></button></li>';
			case 'rooms':
			...
			if (window.nodewebkit) {
				...
				if (!!$favicon.data('on') !== !!notificationCount) {
					if (notificationCount) {
						$favicon.attr('href', Tools.resourcePrefix + '');
						$favicon.data('on', '1');
					} else {
						$favicon.attr('href', Tools.resourcePrefix + '');
						$favicon.data('on', '');
		...
		updateTabbarMini: function () {
			...
			var buf = '<ul><li><a class="button minilogo' + notificationClass + '" href="' + app.root + '"><img src="' + Tools.resourcePrefix + '" width="32" height="32" alt="" /><i class="fa fa-caret-down" style="display:inline-block"></i></a></li></ul>';
	...
	var SoundsPopup = this.SoundsPopup = Popup.extend({
		initialize: function (data) {
			...
			buf += '<p class="effect-volume"><label class="optlabel">Effects volume:</label>' + (silent ? '<em>(silented)</em>' : '<input type="slider" name="effectvolume" value="' + (Tools.prefs('effectvolume') || 50) + '" />') + '</p>';
			buf += '<p class="music-volume"><label class="optlabel">Music volume:</label>' + (silent ? '<em>(silented)</em>' : '<input type="slider" name="musicvolume" value="' + (Tools.prefs('musicvolume') || 50) + '" />') + '</p>';
			buf += '<p class="notif-volume"><label class="optlabel">Notifications volume:</label>' + (muted ? '<em>(silented)</em>' : '<input type="slider" name="notifvolume" value="' + (Tools.prefs('notifvolume') || 50) + '" />') + '</p>';
		...
		setMute: function (e) {
			...
			if (!muted) {
				this.$('.effect-volume').html('<label class="optlabel">Effects volume:</label><input type="slider" name="effectvolume" value="' + (Tools.prefs('effectvolume') || 50) + '" />');
				...
				this.$('.notif-volume').html('<label class="optlabel">Notifications volume:</label><input type="slider" name="notifvolume" value="' + (Tools.prefs('notifvolume') || 50) + '" />');
				this.domInitialize();
			} else {
		...
		update: function () {
			...
			if (app.user.get('named')) {
				...
				if (registered && (registered.userid === app.user.get('userid'))) {
					buf += '<p><button name="changepassword">Change Password</button></p>';
				} else {
					buf += '<p><button name="register">Register Account</button></p>';
				}
			}
			...
			buf += '<hr />';
			if (app.user.get('named')) {
				buf += '<p class="buttonbar" style="text-align:right"><button name="login"><i class="fa fa-pencil"></i>  Desired name</button> <button name="logout"><i class="fa fa-power-off"></i> Sign out</button></p>';
			} else {
				buf += '<p class="buttonbar" style="text-align:right"><button name="login">Desired name</button></p>';
			}
			this.$el.html(buf).css('min-width', 160);
		},
	...
	var AvatarsPopup = this.AvatarsPopup = Popup.extend({
		...
		initialize: function () {
			...
			var buf = '';
			buf += '<p>Switch avatar or <button name="close">Cancel</button></p>';
	...
	var CustomBackgroundPopup = this.CustomBackgroundPopup = Popup.extend({
		...
		initialize: function () {
			...
			buf += '<button name="setBg" value="charizards"' + (cur === 'charizards' ? ' class="cur"' : '') + '><span class="bg" style="background-position:0 -' + (90 * 0) + 'px"></span></button>';
			buf += '<button name="setBg" value="horizon"' + (cur === 'horizon' ? ' class="cur"' : '') + '><span class="bg" style="background-position:0 -' + (90 * 1) + 'px"></span></button>';
			...
			buf += '<button name="setBg" value="ocean"' + (cur === 'ocean' ? ' class="cur"' : '') + '><span class="bg" style="background-position:0 -' + (90 * 3) + 'px"></span></button>';
			buf += '<button name="setBg" value="shaymin"' + (cur === 'shaymin' ? ' class="cur"' : '') + '><span class="bg" style="background-position:0 -' + (90 * 4) + 'px"></span></button>';
			...
			buf += '<p>Drag and drop an image to PWO (the background settings don\'t need to be open), or upload:</p>';
	...
	var LoginPopup = this.LoginPopup = Popup.extend({
		...
		initialize: function (data) {
			...
			buf += '<p><label class="label">Trainer name: <small class="preview" style="' + hashColor(toUserid(name)) + '">(color)</small><input class="textbox autofocus" type="text" name="username" value="' + Tools.escapeHTML(name) + '"></label></p>';
			buf += '<p class="buttonbar"><button type="submit"><strong>Desired name</strong></button> <button name="close">Cancel</button></p>';
	...
	var ChangePasswordPopup = this.ChangePasswordPopup = Popup.extend({
		...
		initialize: function (data) {
			...
			if (data.error) {
				...
				buf += '<p>Create new password:</p>';
			}
			buf += '<p><label class="label">Your name: <strong>' + app.user.get('name') + '</strong></label></p>';
			buf += '<p><label class="label">Recent password: <input class="textbox autofocus" type="password" name="oldpassword" /></label></p>';
			buf += '<p><label class="label">Set password: <input class="textbox" type="password" name="password" /></label></p>';
			buf += '<p><label class="label">Set password (confirm): <input class="textbox" type="password" name="cpassword" /></label></p>';
		...
		submit: function (data) {
			$.post(app.user.getActionPHP(), {
				...
				if (data.actionsuccess) {
					app.addPopupMessage("Password Updated.");
				} else {
					app.addPopup(ChangePasswordPopup, {
	...
	var RegisterPopup = this.RegisterPopup = Popup.extend({
		...
		initialize: function (data) {
			...
			if (data.error) {
				...
				buf += '<p>Protect your account:</p>';
			}
			buf += '<p><label class="label">Desired name: <strong>' + Tools.escapeHTML(data.name || app.user.get('name')) + '</strong><input type="hidden" name="name" value="' + Tools.escapeHTML(data.name || app.user.get('name')) + '" /></label></p>';
			...
			buf += '<p><label class="label"><img src="' + Tools.resourcePrefix + 'sprites/bwani/milotic.gif" /></label></p>';
			...
			buf += '<p class="buttonbar"><button type="submit"><strong>Secure my account</strong></button> <button name="close">Cancel</button></p></form>';
			this.$el.html(buf);
		},
		submit: function (data) {
			var name = data.name;
			...
			$.post(app.user.getActionPHP(), {
				...
				if (data.curuser && data.curuser.loggedin) {
					...
					app.addPopupMessage("You are owner of this account.");
				} else {
					app.addPopup(RegisterPopup, {
						name: name,
						captcha: captcha,
	...
	var LoginPasswordPopup = this.LoginPasswordPopup = Popup.extend({
		...
		initialize: function (data) {
			...
			if (data.error) {
				...
				buf += 'This name is unavailable.</p>';
			}
                        
			buf += '<p>Welcome back<label class="label"><strong>' + Tools.escapeHTML(data.username) + '<input type="hidden" name="username" value="' + Tools.escapeHTML(data.username) + '" /></strong></label></p>';
			buf += '<p>Password:<input class="textbox autofocus" type="password" name="password"></label></p>';
                        buf += '<p class="buttonbar"><button type="submit"><strong>Sign In</strong></button> - <button name="close">Cancel</button></p>';

			buf += '<p class="or"><img src="http://pldh.net/media/pokemon/gen6/xy-animated/384.gif" width="60" height="60"></p>';
			buf += '<p class="buttonbar"><button name="login">Desired name</button></p>';
