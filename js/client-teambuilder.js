(function (exports, $) {
	...
	var TeambuilderRoom = exports.TeambuilderRoom = exports.Room.extend({
		...
		initialize: function () {
			...
			// left menu
			this.$el.addClass('ps-room-transparent').addClass('scrollable');
		...
		updateTeamInterface: function () {
			...
			if (!Storage.whenTeamsLoaded.isLoaded) {
				if (!Storage.whenTeamsLoaded.isStalled) {
					buf = '<div class="pad"><p>Teambuilder</p>';
					buf += '<p>From mayhem: You are trying to run my client lol if you have no idea how to then why are you trying to. Do not ask for help we will not assist you in anyway. </p></div>';
				} else {
					buf = '<div class="pad"><p>Trying to load teams securely.</p>';
					buf += '<p>PWO</p>';
		...
		updateFolderList: function () {
			...
			if (Storage.teams) for (var i = -2; i < Storage.teams.length; i++) {
				...
				case 'gen8': format = 'A' + format.slice(4); break;
		...
		updateTeamList: function (resetScroll) {
			...
			if (!this.curFolder) {
				buf += '<h2>Hey you, yeah you</h2>';
				buf += '<p>I hope you like are game server.?</p>';
				buf += '<p><button class="button" name="greeting" value="Y"><i class="fa fa-smile-o"></i> So, has your day has been great </button> <button class="button" name="greeting" value="N"><i class="fa fa-frown-o"></i> No, Not really :(</button></p>';
				buf += '<h2>All teams</h2>';
			} else {
			...
			if (!teams.length) {
				...
				buf += '<li><p><em>you don\'t have a teams </em></p></li>';
			} else {
				...
				if (!atLeastOne) {
					if (filterFolder) {
						buf += '<li><p><em>you don\'t have any teams in this folder</em></p></li>';
					} else {
		...
		greeting: function (answer, button) {
			...
			if (answer === 'N') {
				buf += '<p>Well you ñeed to smile and be happy. 😀 I hope playing in Pok&eacute;mon World today can help make you happy again!</p>';
			} else if (answer === 'Y') {
				...
				buf += '<p><button class="button" name="greeting" value="W"><i class="fa fa-question-circle"></i> Credits.</button></p>';
			} else if (answer === 'W') {
				buf += '<p>Oh, I\'m Mayhem! I made a Credits button for this...</p>';
		...
		dragStartTeam: function (e) {
			...
			if (document.location.protocol === 'https:') {
				...
				urlprefix = "https://main.pokemon-world.online/action.php?act=dlteam&team=";
			}
