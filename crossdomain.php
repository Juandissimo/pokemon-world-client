...
if (preg_match('/^([a-z0-9-_\.]*?)\.psim\.online$/', $host, $m)) {
	$config['host'] = $m[1];
	if ($config['host'] === 'logs') die; // not authorised
	if ($config['host'] === 'sim') die; // not authorised
} else if ($host === 'testclient.pokemon-world.online') {
		...
		if ($config['host'] !== 'localhost') {
			foreach ($PokemonServers as &$server) {
				if ($config['host'] === $server['server'] && (
					...
					$config['redirect'] = 'http://' . $server['id'] . '.psim.online/' . rawurlencode($path);
					break;
				}
			}
		}
...
var myOrigin = 'https://testclient.pokemon-world.online';
